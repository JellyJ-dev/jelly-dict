...
- Excel 단어장 저장
- 최근 조회 단어 확인 및 검색
- 최근/영어/일본어 단어장 마지막 화면 및 정렬 설정 기억
- 영어 단어장 / 일본어 단어장 검색, 정렬, 확장 보기
- 저장된 단어의 뜻, 예문, 태그, 메모, 출처 상세 확인
- 선택한 단어 수정, 재조회, 삭제 후 되돌리기
- 중복 덮어쓰기/병합 전 Excel 자동 백업
- Anki `.apkg` 파일 내보내기
...
단어장은 `최신순`, `오래된순`, `가나다순`으로 정렬할 수 있으며 마지막으로 열어둔 단어장과 정렬 기준은 다음 실행 때도 유지됩니다.

단어장에서 항목을 삭제하면 하단에 작은 되돌리기 알림이 잠시 표시됩니다. 알림 버튼이나 `Command+Z`로 방금 삭제한 항목을 복원할 수 있습니다.
...
생성된 mp3 파일은 다음 내보내기 속도를 위해 단어장 Excel 파일이 있는 폴더의 `mp3/`에 보관됩니다.
...
| Kokoro TTS | TTS 설치 시 모델 다운로드, 앱 실행 중에는 로컬 캐시만 사용 |
...
- Kokoro: 영어/일본어 지원. 설정의 Kokoro 설치가 모델/영어 G2P/일본어 음소 모듈을 준비하며, 조회·저장·Anki 내보내기 중에는 다운로드하지 않습니다.
- VOICEVOX: 일본어 전용
- edge-tts: 외부 CLI 기반 옵션

VOICEVOX는 캐릭터별 약관에 따라 Anki 덱 설명에 크레딧이 들어갑니다.

`저장 후 TTS 미리 생성`을 켜면 저장 완료 후 백그라운드 큐에서 단어/예문 mp3를 조용히 만듭니다. 검색, 저장, 단어장 스크롤을 막지 않도록 큐는 중복 단어를 최신 항목으로 덮고 오래된 작업을 제한합니다.
